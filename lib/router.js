		...
		case 4: // requested path: /api/dao_factory/1/associated_dao_factory/1
			...
			if (parseInt(identifier) === NaN || associatedIdentifier === NaN || !this.isAllowed(modelName) || !this.isAllowed(associatedModelName)){
				...
				switch(req.method) {
					...
					case 'PUT':
						handleResourceCreateAssociation.call(this, modelName, identifier, associatedModelName, associatedIdentifier, callback)
						break
...
Router.prototype.handleSuccess = function(data, optionsOrCallback, callback) {
  if (typeof optionsOrCallback === 'function') {
    ...
    if(!!data.count || (typeof(data.count) != "undefined" && data.count == 0)){
        callback({
            status: 'success',
            count: data.count,
            data:   data.rows
					...
					entry[association.accessors.get]()
						.success(function(entries){
							...
							if (Array.isArray(entries)) {
								...
								if (entries !== null && entries.values !== undefined) {
					...
					.success(function(child) {
						if (!!child) {
							if (association.associationType === 'BelongsTo' || association.associationType === 'HasOne') {
								parent[association.accessors.get]().success(function (entry) {
									if (entry.id === parseInt(associatedIdentifier)) {
										...
										this.handleError("The object you want to remove from the association is not associated.", callback);
									}
								...
								{
									parent[association.accessors.get]().success(function (entries) {
										var found = false;

										for (var i = 0; i < entries.length; i++) {
											if (entries[i].id === parseInt(associatedIdentifier)) {
												found = true;
												break;
											}
										}
										...
										if (found) {
											...
											this.handleError("No associated object have been found with the specified ID. The association won't be removed.", callback);
										}
									}.bind(this)).error(function(err) {
										this.handleError(err, callback)
									}.bind(this));
							...
							this.handleError("The associated object does not exist !", callback);
						}
					}.bind(this))
					.error(function(err) {
						this.handleError(err, callback)
...
var handleResourceCreateAssociation = function(modelName,identifier,associatedModelName,associatedIdentifier,callback){
	var daoFactory           = this.findDAOFactory(modelName)
		, association          = !!daoFactory ? this.findAssociation(daoFactory, associatedModelName) : null
		, associatedDaoFactory = !!association ? association.target : null
				...
				if (!!parent) {
					associatedDaoFactory
						...
						.success(function (child) {
							if (!!child) {
								if (association.associationType === 'BelongsTo' || association.associationType === 'HasOne') {
									parent[association.accessors.set](child).success(function () {
										this.handleSuccess({}, callback)
									}.bind(this))
										.error(function (err) {
											this.handleError(err, callback)
									...
									parent[association.accessors.add](child).success(function () {
										this.handleSuccess({}, callback)
									}.bind(this))
										.error(function (err) {
											this.handleError(err, callback)
								...
								this.handleError("Wrong id ("+associatedIdentifier+") given for the associated model "+associatedModelName, callback);
							}
						}.bind(this))
						.error(function (err) {
							this.handleError(err, callback)
					...
					this.handleError("Wrong id ("+identifier+") given for the model "+modelName, callback);
				}
			}.bind(this))
			.error(function(err) {
				this.handleError(err, callback)
